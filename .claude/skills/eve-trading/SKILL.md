...
8. **Broker fees break out into new-listing vs. relisting** (`brokerFeesNewListings` / `brokerFeesRelisting`), correlated from `brokers_fee` journal entries against order timestamps — prefer passing `broker_fee_pct` to `run_daily_close` explicitly (Ramon's actual rate, same rule as `get_portfolio_margins`), since the match uses it to compute each order's expected fee. This is a best-effort heuristic, not an exact join — **ESI gives no direct order/fee linkage at all** (confirmed via CCP's own ref_type reference: brokers_fee entries carry no context_id). Whatever can't be confidently matched (no candidate within tolerance, or two orders too close to call) lands in `brokerFeesUnmatched` and gets flagged if it's more than ~20% of the day's fees — `brokerFeesPaid` itself is always exact regardless, only the new/relist split is approximate.
9. **If `broker_fee_pct` is omitted, it's auto-derived from Ramon's own paid-fee history** (`get_effective_broker_fee_pct`, or automatically inside `run_daily_close`) rather than defaulting to a generic rate — it finds unambiguous 1:1 fee/order pairs and takes the median observed rate. Deliberately not computed from the game's skill+standings formula: that would need a new ESI scope (standings) this server doesn't request, and drifts if CCP rebalances the formula, whereas the derived rate self-corrects automatically. Check the close's `flags` to see whether the rate used was provided, derived (and from how many samples), or fell back to the generic default from too little history — treat a default-fallback close's new/relist split with more skepticism.
