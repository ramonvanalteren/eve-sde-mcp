...
- **But "re-verify" can itself lie — ESI data is cached, not always live.** `get_region_orders` and `price_build`'s live pricing are both backed by ESI's region market-orders endpoint, which caches for up to **300 seconds (5 minutes)** — confirmed 2026-09 by reading ESI's own `Expires`/`Last-Modified` response headers directly, and matching a CCP developer-forum statement ("the Cache timer is 300 seconds"). Other tools this skill touches cache longer: `get_character_blueprints` and `get_character_assets` (so a just-delivered ME/TE research job's new level may not show for up to an hour), `get_industry_jobs` (not independently confirmed — treat conservatively as potentially stale too). `get_market_history` is different again: it refreshes once daily at ~11:05 UTC after downtime, not a rolling 5-minute window.
  - **The tell**: a repeated, byte-identical result (same order ID, same issue timestamp, same volume) across two calls made minutes apart means you're reading a cache, not the live book — a genuinely active market never returns the exact same snapshot twice.
  - **When a number looks surprising** (a sudden margin collapse, a price that contradicts what the user reports seeing in the client), don't treat the tool as automatically more current — ask, or trust the user's direct in-client observation over the tool's output, and say so plainly rather than re-running the same call expecting a different answer inside the cache window.
  - **Confirmed case**: `get_region_orders` reported a fresh sell order dropping Valkyrie I's price ~20% (37,290 → 29,610 ISK), turning a verified +13.7% margin on a 400-run batch negative. Two consecutive calls returned the identical order ID and timestamp. The user, physically in Jita, saw no such order — lowest sell was still 37,290. The read was stale; nothing was actually wrong with the line.
